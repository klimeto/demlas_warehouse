(function (win, doc) {
	...
	map.on('singleclick', function (evt) {
		...
		if (poi_enabled) {
			...
			$("#md-info-body").html("<p>Rezultati preklapanja točke interesa: <br>" + HDMS + "<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
			...
			$('#click-info-results').html('<p>Kliknuli ste ovdje:<br>LAT: ' + LAT + '<br>LON: ' + LON + '</p><p>Ako želite koristiti ovu poziciju kao upit omogućite opciju Točka interesa</p>');
		}
	...
	$('#btn-search').click(function (e) {
		...
		if (bbox_enabled && searchTerm === '') {
			...
			console.log(url);
			$('#md-info').modal('show');
			$("#md-info-body").html("<p>Rezultati preklapanja graničnog okvira: <br> " + bounds[0] + ", " + bounds[1] + ", " + bounds[2] + ", " + bounds[3] + "<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
		}
		if (bbox_enabled && searchTerm != '') {
			var bounds = ol.proj.transformExtent(map.getView().calculateExtent(map.getSize()), 'EPSG:3857', 'EPSG:4326');
			console.log(bounds);
		...
		if (searchTerm != '' && (!bbox_enabled && !geoloc_enabled)) {
			...
			$('#md-info').modal('show');
			$("#md-info-body").html("<p>Rezultati pretraživanja: <br>'" + searchTerm + "'<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
			//doSearch(searchTerm);
			$("#srch-term").val('');
		...
		if (searchTerm === '' && (!bbox_enabled && !geoloc_enabled)) {
			...
			$('#md-info').modal('show');
			$("#md-info-body").html("<p>"+ searchText +"<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
			//doSearch(searchTerm);
			$("#srch-term").val('');
		}
		...
		if (geoloc_enabled && searchTerm === '') {
			...
			$('#md-info').modal('show');
			//$('.modal-title').append("<p>Results covering location with Latitude:" + LAT +" and Longiture" + LON +"</p>");
			$("#md-info-body").html("<p>Rezultati pokrivanja trenutne geografske lokacije:<br><b>Longitude: " + x + "<br>Latitude: " + y + "<br><iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
		}
	...
	$('#srch-term').keypress(function (e) {
		...
		if (e.which == '13') {
			...
			if (bbox_enabled && searchTerm != '') {
				...
				console.log(url);
				$('#md-info').modal('show');
				$("#md-info-body").html("<p>Rezultati preklapanja graničnog okvira: <br>" + bounds[0] + ", " + bounds[1] + ", " + bounds[2] + ", " + bounds[3] + "<br>Search term: " + searchTerm + "<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
			}
			if (searchTerm != '' && (!bbox_enabled && !geoloc_enabled)) {
				var url = gnk + '?resultType=details&fast=index&from=1&to=20&sortBy=relevance&any=' + searchTerm + category;
				$('#md-info').modal('show');
			...
			if (searchTerm === '' && (!bbox_enabled && !geoloc_enabled)) {
				var url = gnk + '?any=' + category;
				$('#md-info').modal('show');
				$("#md-info-body").html("<p><iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
				//doSearch(searchTerm);
				$("#srch-term").val('');
			}
